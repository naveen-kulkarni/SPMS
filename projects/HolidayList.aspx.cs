                ...
                for (int i = 0; i < e.Row.Cells.Count; i++)
                {
                    ...
                    else
                    {
                        if (i != 0 && e.Row.Cells[i].Text.Replace("&nbsp;", "").Trim() != "")
                        {
                            ...
                            string[] splittedValue = e.Row.Cells[i].Text.Split('-');
                            DateTime dtsplittedValue = DateTime.Parse(splittedValue[0].ToString());
                            lblDate.Text = dtsplittedValue.Day.ToString();
                            ...
                            if (dsHolidayList.Tables[0].Rows.Count > 0)
                            {
                                ...
                                {
                                    DateTime dtHoliday = Convert.ToDateTime(dsHolidayList.Tables[0].Rows[j]["HolidayDate"].ToString());
                                    if (dtHoliday.ToShortDateString().Equals(dtAllDate.ToShortDateString()))
                                    {
                                        e.Row.Cells[i].BackColor = ColorTranslator.FromHtml("#6CE26C");
                                        lblDate.ToolTip = dsHolidayList.Tables[0].Rows[j]["Holiday_Name"].ToString();
                                    }

                                }
                            }
                            e.Row.Cells[i].Controls.Add(lblDate);
                        }
                        else if (i == 0)
                        {
